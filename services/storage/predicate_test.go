...
func TestHasSingleMeasurementNoOR(t *testing.T) {
	cases := []struct {
		...
		{
			expr: influxql.MustParseExpr(`_measurement = 'm0'`),
			name: "m0",
			ok:   true,
		},
		{
			expr: influxql.MustParseExpr(`_something = 'f' AND _measurement = 'm0'`),
			name: "m0",
			ok:   true,
		},
		{
			expr: influxql.MustParseExpr(`_something = 'f' AND (a =~ /x0/ AND _measurement = 'm0')`),
			name: "m0",
			ok:   true,
		},
		{
		...
		{
			expr: influxql.MustParseExpr(`_measurement = 'm0' OR tag1 != 'foo'`),
			ok:   false,
		},
		{
			expr: influxql.MustParseExpr(`_measurement = 'm0' AND tag1 != 'foo' AND _measurement = 'other'`),
			ok:   false,
		},
		{
			expr: influxql.MustParseExpr(`_measurement = 'm0' AND tag1 != 'foo' OR _measurement = 'other'`),
			ok:   false,
		},
		{
			expr: influxql.MustParseExpr(`_measurement = 'm0' AND (tag1 != 'foo' OR tag2 = 'other')`),
			ok:   false,
		},
		{
			expr: influxql.MustParseExpr(`(tag1 != 'foo' OR tag2 = 'other') OR _measurement = 'm0'`),
			ok:   false,
		},
	}
